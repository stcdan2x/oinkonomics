...
Tap **Health event** (the card is titled **Health event (whole batch)** because one entry covers every pig in the batch). Choose the **Type**: **Treatment (medicine)**, **Deworming**, **Vaccination** or **Iron shot**. Pick the **Product** from the list of products with known withdrawal periods, or **Other (type the name)** and fill in **Product name**. Enter the **Date given** and the **Withdrawal days**: the list fills this in for known products, and for anything else copy it from the product label (0 if the label states none). The form shows the earliest sale date after this dose. **Dose** and **Note** are optional. **Save**.
...
- Feed and medicine used are not ledger entries at all: the money left when the stock was bought. Their use reaches the batch costing through the daily feed entry instead.
...
Each entry takes the feed out of stock and charges it, at the unit cost of that moment, to the batch it was fed to; feed for the breeding herd becomes part of the herd's cost and reaches the piglets at weaning. Feed recorded here is the biggest single line in every batch's costing, see [manage a batch](/guide/manage-a-batch) and [cost allocation](/guide/cost-allocation). Medicines used are recorded as health events on the batch or animal, not here; their cost reaches the batch the same way only when a move is recorded against them, so record a **Loss** or a count adjustment for a vial that was used up.

## Counting, losses and the item page

Open an item to see **On hand**, **Reorder level**, **Unit cost**, **Value on hand** and **Expiry**, the **Edit item** button for the fields above, and the list of every move: **Purchase**, **Used**, **Adjustment** and **Loss**, each with its date, the batch it went to, and the value at cost. When the sack count on the floor differs from the app, tap **Adjust or record loss**, choose **Physical count differs (set the new count)** and enter what you counted; for feed that spoiled, spilled or expired choose **Loss: spoiled, spilled or expired** and the amount. A move that was a mistake can be removed with **Delete** and **Confirm delete**; removing a purchase also removes its ledger entry, so the ledger and the stock stay in step.
